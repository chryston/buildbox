...
function computeCutListForUnit(unit: CabinetSceneUnit): CutListEntry[] {
  ...
  raw.push({ label: 'Back panel', qty: 1, width: gs.width - 2 * t, height: gs.height - 2 * t, depth: backThickness, material: gs.defaultMaterial, notes: `${backThickness}mm ply back` })

  if (gs.toeKick) {
    raw.push({ label: 'Toe-kick board', qty: 1, width: gs.width - 2 * gs.toeKick.setback, height: gs.toeKick.height, depth: t, material: gs.defaultMaterial })
  }
